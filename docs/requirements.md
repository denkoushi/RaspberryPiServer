...
| --- | --- | --- | --- |
| DocumentViewer 移行 | ⚙ 稼働中（Pi5 で `/viewer`・Socket.IO を提供） | ✅ 2025-11-02: Window A の systemd drop-in / `.env` を Pi5 向けに確定し、`config/raspi-server.env.sample` と RUNBOOK に反映（`SOCKET_STATUS_WATCHDOG`・トークン整合を明文化）<br>✅ 2025-11-02: Pi4 実機で Playwright ライブシナリオを実施し、所在サマリー・構内物流更新が自動検証可能であることを確認（記録: `docs/test-notes/2025-11-02-window-a-live-playwright.md`）<br>✅ 2025-11-02: 14 日連続チェック用テンプレートに DocumentViewer 観点を追記し、`docs/test-notes/2025-11-01-14day-check.md` を更新<br>✅ 2025-11-02: ステータスバー UI のレイアウト／アクセシビリティ調整を完了し、Pi4 実機で Playwright ライブシナリオ再実行（記録更新: `docs/test-notes/2025-11-02-window-a-live-playwright.md`、要件: `docs/requirements/window-a-statusbar.md`） | `docs/documentviewer-migration.md`, `DocumentViewer/docs/test-notes/2025-10-26-viewer-check.md` |
| 工具管理 UI クライアント化 | ⏳ 進行中（UI/REST プロキシ集約を設計） | 1. Window A の API 参照先を Pi5 へ統一し、旧サーバー経由コードを削除（2025-11-02: `/api/loans`・`register_*`・`tool_name` 系 API を RaspberryPiServer に実装し、Window A からのリクエストをプロキシ化）<br>2. Pi4 で NFC スキャンを継続できるよう、`ENABLE_LOCAL_SCAN` を既定有効とした環境テンプレート・ドキュメント整備<br>3. Socket.IO / REST テストログを `docs/test-notes/` へ追記（2025-11-02 viewer highlight 実機検証を記録済）<br>4. 2025-11-02: `tests/e2e/window-a-live.spec.ts` を整備し、Pi4 実機上でスキャン→所在サマリー→物流タブ更新のライブ Playwright テストが成功した（`.env.test` 経由で実行）。 | Window A `docs/right-pane-plan.md`, `docs/implementation-plan.md` |
