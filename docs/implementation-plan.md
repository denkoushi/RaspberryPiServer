...
- Docker/PostgreSQL セットアップ（`docker-compose.yml`、`.env` 整備、バックアップ連携）— ローカル環境では Docker デーモン未起動のため未検証。Pi 上での確認手順は `docs/test-notes/2025-10-25-postgres-compose.md` を参照。
