...
### 1.3 リモート／実機更新の役割分担（重要）
- **Git のリモートコミットと Raspberry Pi 実機の反映作業は常にユーザーが実施する。エージェントは決してリモートコマンドを実行できないし、実機を直接操作できない。**
- エージェントはコマンド例や手順をコードブロックで提示し、結果共有を待って次の指示を出す。その際も「実行済み」と断定せず、常にユーザーが操作した前提で状況確認を行う。
- ラズベリーパイ更新手順を伴うタスクでは、`git pull` を含むステップを **必ず順序立てて案内** し、ユーザーからの実行報告を待ってから次の指示へ進む。
